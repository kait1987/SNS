...
2. **Supabase 클라이언트 파일들** (`lib/supabase/`):
   ...
   **Supabase 공식 방식** (Supabase 자체 인증 사용 시):
   - `client-official.ts`: Client Component용 (createClient)
     - Cookie-based 인증, `@supabase/ssr` 패키지 사용
   - `server-official.ts`: Server Component용 (createClient)
     - Cookie-based 인증, Next.js 15 App Router 최적화
   ...
   **참고**: 각 클라이언트의 사용 방법은 `lib/supabase/README.md` 참고
