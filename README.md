...
### 🗄️ 데이터베이스 통합
- **Clerk 통합 방식** (권장): Clerk 토큰 기반 Supabase 인증
  - Client Component용 (`useClerkSupabaseClient`)
  - Server Component용 (`createClerkSupabaseClient`)
- **Supabase 공식 방식**: Cookie-based 인증 (Supabase 자체 인증 사용 시)
  - Client Component용 (`createClient` from `client-official.ts`)
  - Server Component용 (`createClient` from `server-official.ts`)
...
자세한 통합 가이드는 [Clerk + Supabase 통합 가이드 문서](docs/CLERK_SUPABASE_INTEGRATION.md)를 참고하세요.

[Clerk 공식 통합 가이드](https://clerk.com/docs/guides/development/integrations/databases/supabase)에서도 추가 정보를 확인할 수 있습니다.
...
- `/instruments`: Supabase 공식 가이드 예시 (Cookie-based 인증)
...
- `lib/clerk/localization.ts`: 커스텀 한국어 로컬라이제이션 정의
- `app/layout.tsx`: `ClerkProvider`에 커스텀 로컬라이제이션 적용
...
> **개발 단계**: 현재 프로젝트는 개발 편의를 위해 RLS가 비활성화되어 있습니다. 프로덕션 배포 전에는 반드시 적절한 RLS 정책을 설정해야 합니다.

RLS 정책 예시는 `supabase/migrations/example_rls_policies.sql` 파일을 참고하세요.

**Clerk 통합 시 RLS 정책 작성 방법:**

```sql
-- Clerk user ID는 auth.jwt()->>'sub'로 접근
...
자세한 내용은 [Clerk + Supabase 통합 가이드](docs/CLERK_SUPABASE_INTEGRATION.md)를 참고하세요.
...
│   │   ├── server.ts             # Clerk 통합 (Server Component용)
│   │   ├── client-official.ts    # Supabase 공식 방식 (Client Component용)
│   │   ├── server-official.ts    # Supabase 공식 방식 (Server Component용)
...
│   │   ├── setup_storage.sql         # Storage 버킷 설정
│   │   ├── create_instruments_table.sql  # Supabase 공식 가이드 예시
│   │   └── example_rls_policies.sql  # RLS 정책 예시
...
- **`lib/supabase/`**: 환경별 Supabase 클라이언트 (매우 중요!)
  - **Clerk 통합**: `clerk-client.ts`, `server.ts`
  - **Supabase 공식 방식**: `client-official.ts`, `server-official.ts`
  - **기타**: `service-role.ts`, `client.ts`
  - **가이드**: `README.md` (클라이언트 선택 가이드)
